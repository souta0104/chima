...
6. 置き場所: 専用の新規リポジトリ
7. Claude Code の権限モードは auto。Codex は workspace-write、承認なし、
   CHIMA_HOME の追加書き込み、ネットワーク許可で起動する。承認プロンプトに当たったら
...
- 人間がボールを持つイシューは assignee = 人間、delegate なし。人間の対応が
  必要な作業は、コメント本文だけの依頼で終わらせず、必ず人間 assignee のイ
  シューとして存在させる
- 未着手・ブロック中 (Todo / Backlog) は assignee・delegate ともになし
- status の運用は以下に統一する (2026-07-11 確定):
